...
This tutorial is a guided first run of `ss.shift.detect_harm(...)`.
...
- Use `ss.shift.detect_harm(...)` and set direction correctly
...
`ss.shift.detect_harm` needs one score per sample that encodes how adverse that sample is.
...
### Step 2 — Run the harmful-shift test

Pass the source sample first, then the target sample. `ss.shift.detect_harm(...)` tests whether `bowl`
...
print(f"Harmful-shift p-value: {harm.pvalue:.4f}")
```

**Output:**

```text
Harmful-shift p-value: 0.1215
```
...
- **Pair with shift testing:** Run `ss.shift.detect_shift(...)` first to detect *any* change, then run `ss.shift.detect_harm(...)` to decide
  whether the change is harmful. See the [credit risk how-to](../credit/monitor-credit-risk.md) for a full
