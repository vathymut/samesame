...
You do not need to compute these by hand. `common_support_weights(...)` does it for you.
...
In all three cases, `common_support_weights(...)` normalizes each active group so the weights
