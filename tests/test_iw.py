...
def test_source_mode_reweights_source_only(domain_probabilities):
    ...
    result = domain_weights(**domain_probabilities, mode="source")
...
def test_target_mode_reweights_target_only(domain_probabilities):
    ...
    result = domain_weights(**domain_probabilities, mode="target")
...
def test_both_mode_reweights_all(domain_probabilities):
    ...
    result = domain_weights(**domain_probabilities, mode="both")
...
def test_default_mode_is_both(domain_probabilities):
    ...
    both_result = domain_weights(**domain_probabilities, mode="both")
...
def test_invalid_lambda_too_low(domain_probabilities):
    with pytest.raises(ValueError, match="lambda_ must be in"):
        domain_weights(**domain_probabilities, lambda_=-0.1)


def test_invalid_lambda_too_high(domain_probabilities):
    with pytest.raises(ValueError, match="lambda_ must be in"):
        domain_weights(**domain_probabilities, lambda_=1.1)


def test_invalid_lambda_nan(domain_probabilities):
    with pytest.raises(ValueError, match="lambda_ must be in"):
        domain_weights(**domain_probabilities, lambda_=np.nan)
...
def test_invalid_mode_raises(domain_probabilities):
    with pytest.raises(ValueError, match="mode must be one of"):
        domain_weights(**domain_probabilities, mode="not-a-mode")  # type: ignore[arg-type]
...
def test_empty_source_prob_raises():
    with pytest.raises(ValueError, match="non-empty"):
        domain_weights(source_prob=np.array([]), target_prob=np.array([0.5]))


def test_empty_target_prob_raises():
    with pytest.raises(ValueError, match="non-empty"):
        domain_weights(source_prob=np.array([0.5]), target_prob=np.array([]))
...
def test_ess_lambda_one_gives_sample_size(domain_probabilities):
    ...
    result = domain_weights(**domain_probabilities, mode="both", lambda_=1.0)
    ess = result.effective_sample_size()
