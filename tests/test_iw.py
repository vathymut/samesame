...
common_support_weights = domain_weights
...
def test_source_mode_reweights_source_only(domain_probabilities):
    ...
    result = common_support_weights(**domain_probabilities, reweight="source")
...
def test_target_mode_reweights_target_only(domain_probabilities):
    ...
    result = common_support_weights(**domain_probabilities, reweight="target")
...
def test_both_mode_reweights_all(domain_probabilities):
    ...
    result = common_support_weights(**domain_probabilities, reweight="both")
...
def test_default_mode_is_both(domain_probabilities):
    ...
    default_result = common_support_weights(**domain_probabilities)
    both_result = common_support_weights(**domain_probabilities, reweight="both")
...
def test_lambda_one_gives_uniform(domain_probabilities):
    ...
    result = common_support_weights(
        **domain_probabilities, reweight="source", shrinkage=1.0
    )
    assert np.allclose(result.source, [1.0, 1.0])
    assert np.allclose(result.target, [1.0, 1.0])
...
def test_lambda_zero_gives_plain_density_ratio(domain_probabilities):
    ...
    result = common_support_weights(
        **domain_probabilities, reweight="source", shrinkage=0.0
    )
...
def test_invalid_domain_prob_below_zero():
    with pytest.raises(ValueError, match="domain probabilities"):
        common_support_weights(
            source=np.array([-0.1, 0.5]), target=np.array([0.5, 0.5])
        )


def test_domain_prob_at_zero_is_clipped():
    common_support_weights(
        source=np.array([0.0, 0.5]), target=np.array([0.5, 0.5])
    )


def test_domain_prob_at_one_is_clipped():
    common_support_weights(
        source=np.array([0.5, 0.5]), target=np.array([0.5, 1.0])
    )
...
def test_invalid_lambda_too_low(domain_probabilities):
    with pytest.raises(ValueError, match="shrinkage must be in"):
        common_support_weights(**domain_probabilities, shrinkage=-0.1)


def test_invalid_lambda_too_high(domain_probabilities):
    with pytest.raises(ValueError, match="shrinkage must be in"):
        common_support_weights(**domain_probabilities, shrinkage=1.1)


def test_invalid_lambda_nan(domain_probabilities):
    with pytest.raises(ValueError, match="shrinkage must be in"):
        common_support_weights(**domain_probabilities, shrinkage=np.nan)
...
def test_invalid_mode_raises(domain_probabilities):
    with pytest.raises(ValueError, match="reweight must be one of"):
        common_support_weights(**domain_probabilities, reweight="not-a-mode")  # type: ignore[arg-type]
...
def test_empty_source_prob_raises():
    with pytest.raises(ValueError, match="non-empty"):
        common_support_weights(source=np.array([]), target=np.array([0.5]))


def test_empty_target_prob_raises():
    with pytest.raises(ValueError, match="non-empty"):
        common_support_weights(source=np.array([0.5]), target=np.array([]))
...
def test_output_shape_and_dtype(domain_probabilities):
    result = common_support_weights(**domain_probabilities)
...
def test_ess_lambda_one_gives_sample_size(domain_probabilities):
    ...
    result = common_support_weights(**domain_probabilities, reweight="both", shrinkage=1.0)
    ess = result.effective_sample_size()
