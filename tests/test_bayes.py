...
from samesame import stats


def test_conversion(bayes_factors):
    pvalues = stats.as_pvalue(bayes_factors)
    round_trip = stats.as_bf(pvalues)
...
def test_as_pvalue_scalar():
    ...
    bayes_factor = 1.0
    expected_pvalue = 0.5
    result = stats.as_pvalue(bayes_factor)
...
def test_as_pvalue_array():
    ...
    result = stats.as_pvalue(bayes_factors)
...
def test_as_pvalue_invalid_bf():
    ...
    for bf in invalid_bfs:
        with pytest.raises(ValueError, match=error_msg):
            stats.as_pvalue(bf)
    with pytest.raises(ValueError, match=error_msg):
        stats.as_pvalue(np.array(invalid_bfs))
...
def test_bayes_factor_scalar():
    ...
    threshold = 0.5
    expected_bf = 1.0
    result = stats.bayes_factor(posterior, threshold)
    assert np.isclose(result, expected_bf), f"Expected {expected_bf}, got {result}"
...
def test_as_bf_invalid_pvalues():
    ...
    for pvalue in invalid_pvalues:
        with pytest.raises(ValueError, match=error_msg):
            stats.as_bf(pvalue)
    with pytest.raises(ValueError, match=error_msg):
        stats.as_bf(np.array(invalid_pvalues))
