...
permutation minimum $p = 0.0001$. Most of each population lies outside the common
support. The doubly weighted test, which adjusts both sides, does not reject
($p = 0.992$). The original rejection was driven by context change---applicant
...
\begin{figure}[t]
	...
	dashed line marks the 0.05 decision threshold. The three standard modes
	(unweighted, source-weighted, target-weighted) reject at the permutation
	minimum, and the causal-inference baselines (Crump-trimmed,
	overlap-weighted) also reject; the doubly weighted test does not.
	Effective sample sizes after reweighting to common support:
	source 310 of 1,536; target 337 of 2,776. The non-rejection indicates
...
($p = 0.131$)---a non-rejection that common-support restriction does not
