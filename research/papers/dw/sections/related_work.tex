...
Cobb and Van Looveren~\cite{pmlr-v162-cobb22a} test two-sided equality of conditionals, $H_0: P(S^0|C=c)=P(S^1|C=c)$ $P_{C_1}$-a.e., via a maximum mean discrepancy (MMD)-based average distributional treatment effect on the treated (ADiTT) that kernel-weights comparisons toward deployment-supported contexts; their MMD-Sub baseline, which rejection-samples the reference to match the deployment context distribution before testing, is the closest precedent for our common-support reframing. Hu and Lei~\cite{Hu_2024} developed a conformal conditional distribution test. Xu et al.~\cite{Xu_2024_crt} corrected the conditional randomization test using importance weights. These methods ask whether the \emph{relationship} between features and outcome changed---complementary to whether the outcome distribution itself worsened, which is our one-sided question.
