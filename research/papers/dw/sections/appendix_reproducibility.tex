...
\label{app:lambda}
\input{sections/appendix_lambda}
...
\item Real-data workflow study: four mirrored tasks (HELOC, diabetes readmission, ACS income, and ACS public coverage) plus the NSW employment experiment (Dehejia--Wahba files from the NBER mirror: screened CPS comparison sample as source, NSW treated group as target); ten-fold domain-classifier CV; at most $30{,}000$ source-train rows and $4{,}000$ held-out source plus $4{,}000$ target evaluation rows per task; $9{,}999$ permutation resamples per test; seed $123456$; default $\lambda = 0.5$.
\end{itemize}

The real-data workflow study reports predicted risk, model confidence, and prediction error from one source-trained model per task. The metadata JSON records the exact OpenML IDs, split rules, task sizes, script arguments, and package commit. \emph{Snapshot note:} committed \texttt{results/} and \texttt{figures/} were regenerated at the current protocol ($9{,}999$ permutation resamples, ten-fold domain-classifier CV, NBER-backed NSW loader); metadata hashes match the current tree. The planned HELOC robustness checks (alternative classifier, perturbed split) remain future work (\S\ref{app:domain-clf-sensitivity}).
...
\begin{itemize}
    ...
    \item Permutation inference uses $499$ resamples for the synthetic experiments ($999$ for the domain-classifier study, $199$ for the second DGP); the real-data workflow uses $9{,}999$. Sensitivity to resample count is verified in \S\ref{app:perm-sensitivity}.
