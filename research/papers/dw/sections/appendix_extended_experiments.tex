...
\paragraph{Real-data classifier and split robustness (pre-registration for revision).} The synthetic sensitivity study isolates classifier choice from feature dimensionality. For the revision we will repeat the HELOC real-data workflow with an alternative domain classifier (random forest with $100$ trees) and with a perturbed split variable (ACS Income re-split on a different \texttt{DIVISION} threshold) to verify that the $401/803$ ESS and the $p=0.376$ non-rejection are not artifacts of a single classifier or split definition. Committed \texttt{results/} will be regenerated from the rebased branch before submission; until then \texttt{verify\_result\_metadata} reports the current artifacts as pre-stale (see Reproducibility Plan).
