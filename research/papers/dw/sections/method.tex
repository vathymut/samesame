...
\end{align}
The total effective sample for the doubly weighted test is $\min(\mathrm{ESS}_{\mathrm{src}}, \mathrm{ESS}_{\mathrm{tgt}})$. Smaller $\lambda$ concentrates weights on fewer observations and lowers ESS; larger $\lambda$ retains more sample mass at the cost of bias toward the unweighted estimand. The bias $\|\Theta_\lambda - \Theta\|$ is $O(1-\lambda)$ under $L^2$ convergence of $\hat{r}$ to $r$. In the HELOC example (\S\ref{sec:introduction}), the doubly weighted ESS drops to $310$ of $1{,}536$ source and $337$ of $2{,}776$ target observations---reflecting the limited overlap that drives the unweighted rejection. The density-ratio literature~\cite{Yamada_2013} recommends $\lambda=0.5$ as a practical default; all experiments use $\lambda=0.5$ unless otherwise noted. Section~\ref{sec:results} evaluates this approach under weak overlap, power, and asymmetric contamination across synthetic and real-world tasks. The theoretical context for the bias bound is given in Appendix~\ref{app:theory}.
