...
The real-data workflow mirrors this stability. For HELOC predicted risk, the doubly weighted test does not reject anywhere on the grid ($p = 0.9999$ at $\lambda = 0.25$, $p = 0.992$ at $\lambda = 0.5$, $p = 0.816$ at $\lambda = 0.75$), while the unweighted rejection persists at every $\lambda$; the corresponding runs are recorded in \texttt{results/}.
