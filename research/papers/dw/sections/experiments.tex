...
\input{tables/calibration_rejection_table}
...
We evaluate on four tasks from the TableShift benchmark~\cite{Gardner_2023}---HELOC (credit risk), hospital readmission, ACS income, and ACS public coverage---covering credit, healthcare, and socioeconomic domains across three monitoring profiles per task (risk, confidence, error; 48 tests), plus the National Supported Work (NSW) employment experiment~\cite{LaLonde_1986} as a discrete-score complement.\footnote{Three additional TableShift tasks are blocked by OpenML mirror issues (PhysioNet checksum, College Scorecard Carnegie column, MIMIC LOS insurance/LOS columns), not by the method; synthetic experiments (5 setups $\times$ 100 repeats) provide the primary statistical evidence.} For each task, we drop the split variable before training and test the three profiles.
