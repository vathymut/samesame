...
TASK_SPECS = {
    ...
    "nsw": TaskSpec(
        ...
        data_source="DoWhy Lalonde (Dehejia-Wahba NSW, 445 obs) via NBER nswre74",
        source_definition="treat == 0 (NSW control group, n=260)",
        target_definition="treat == 1 (NSW treatment group, n=185)",
        label_definition="re78 (continuous post-program earnings, USD 1978)",
        narrative_role="motivation",
        openml_dataset_name="",
        openml_data_id=0,
        status="ready",
        note="LaLonde (1986) NSW employment experiment; Python source via dowhy.datasets.lalonde_dataset (NBER nswre74_control/treated), re78 kept continuous",
    ),
}
