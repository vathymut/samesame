...
ALPHA = 0.05
OFF_SUPPORT_GRID = (0.0, 0.1, 0.2, 0.3, 0.4)


def single_run(off_support_fraction, effect, i, n_source, n_target,
               n_resamples, seed):
    s = seed + i
    rng = np.random.default_rng(s)
    source_score, target_score, source_feature, target_feature = gen_overlap(
        rng, n_source, n_target, off_support_fraction=off_support_fraction,
        effect=effect)
    ru = ss.test_harm(
    ...
    row = {
        "run": i, "off_support_fraction": off_support_fraction,
        "effect": effect,
...
def run_setting(off_support_fraction, effect, n_runs, n_source, n_target,
    ...
    for i in range(n_runs):
        if i in done:
            continue
        row = single_run(off_support_fraction, effect, i, n_source,
                         n_target, n_resamples, seed)
        rows.append(row)
        if outpath is not None:
            pd.DataFrame([row]).to_csv(
                outpath, mode="a", header=not outpath.exists(), index=False)
        print(f"[osp={off_support_fraction}/eff={effect}] "
              f"run {i + 1}/{n_runs} done", flush=True)
...
def main():
    ...
    ap.add_argument("--off-support-fraction", type=float, nargs="*",
                    default=list(OFF_SUPPORT_GRID))
    ...
    all_frames = []
    for osp in args.off_support_fraction:
        if args.n_runs == 0:
            continue
        tag = f"{osp}".replace(".", "p")
        outpath = outdir / f"expov_osp{tag}.csv"
        df = run_setting(osp, args.effect, args.n_runs, args.n_source,
                         args.n_target, args.n_resamples, args.seed,
                         outpath=outpath, overwrite=args.overwrite)
        all_frames.append(df)
        summary["settings"][f"{osp}"] = {
        ...
        st = summary["settings"][f"{osp}"]
        print(f"osp={osp}: rej_u={st['reject_unweighted']:.2f} "
