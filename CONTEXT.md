...
**Test ordering (unresolved)**:
For most experiments where one arm is expected to win, running `test_model_lift` first (weaker claim: does the estimator predict degree of benefit better than a reference?) then `test_subgroup_effect` (stronger claim: do genuine crossover subgroups exist?) is the natural sequence. Watson & Holmes (2020) may prescribe a specific ordering — confirm against the paper before prescribing this sequence in documentation or tutorials.
_Avoid_: Prescribing a fixed order in docs before confirming against the paper
...
The two RCT validation modules get three doc entries: (1) a shared Tutorial page — *"Validate a pricing experiment"* — in the Tutorials nav section, covering both `test_subgroup_effect` and `test_model_lift` as a natural workflow; (2) an API reference page `api/subgroup.md` for `samesame.subgroup`; (3) an API reference page `api/model_selection.md` for `samesame.model_selection`. Both API pages mirror `api/testing.md` in structure. No How-to guide in v1. The `site_description` in `mkdocs.yml` must be updated to cover RCT validation alongside distribution shift.
_Avoid_: Giving `samesame.model_selection` its own separate Tutorial; adding either module to How-to guides only; skipping the Tutorial; leaving the site_description shift-only
