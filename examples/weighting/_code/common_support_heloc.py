...
w_target = ss.domain_weights(source=source_prob, target=target_prob, reweight="target", shrinkage=0.5)
w_common_support = ss.domain_weights(source=source_prob, target=target_prob, reweight="both", shrinkage=0.5)
...
target_weighted = ss.test_harmful_shift(source=train_risk, target=deployment_risk, worse="higher", weights=w_target, rng=np.random.default_rng(12345))
common_support = ss.test_harmful_shift(source=train_risk, target=deployment_risk, worse="higher", weights=w_common_support, rng=np.random.default_rng(12345))
